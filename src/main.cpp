...
int64 nChainStartTime = 1401769319;
int nCoinbaseMaturity = 340;
...
const string strMessageMagic = "MachineryCoin Signed Message:\n";
...
static const int CUTOFF_HEIGHT = POW_CUTOFF_HEIGHT;	// Height at the end of 30 days
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
     ...
     if(nHeight == 1)
	{
      ...
      else if(nHeight >1 && nHeight <= 7200)
	{
		nSubsidy = 1000 * COIN;
	}
    else if(nHeight >7200 && nHeight <= 14400)
	{
		nSubsidy = 800 * COIN;
	}
 ...
 else if(nHeight >21600 && nHeight <= 28800)
	{
		nSubsidy = 400 * COIN;
	}
	 else if(nHeight >28800 && nHeight <= 36000)
	{
		nSubsidy = 300 * COIN;
	}
	 else if(nHeight >36000 && nHeight <= CUTOFF_HEIGHT)
	{
		nSubsidy = 200 * COIN;
	}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in MachineryCoin
    bool fStrictPayToScriptHash = true; // Always active in MachineryCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MachineryCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
      if (!fAllowNew)
        ...
        // Genesis block
        const char* pszTimestamp = "31-May-2014 the coins coming from USA ";
        CTransaction txNew;
       ...
       if (false ) {
        ...
        assert(block.hashMerkleRoot == uint256("0x503cf7b62418bac81736e2abe0da9a64126eea31227d18e45ffb6353623a4f53"));
		 block.print();
...
unsigned char pchMessageStart[4] = { 0xab, 0xca, 0xba, 0xac };
